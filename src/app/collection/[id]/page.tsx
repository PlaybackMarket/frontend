...
export default function CollectionDetailPage() {
  ...
  const loadCollectionData = async () => {
    ...
    try {
      ...
      // Fetch active lending listings from the lending service
      // For now, we'll create sample lending offers based on the NFTs
      // In a real implementation, this would come from your lending protocol
      const offers: LendingOffer[] = nfts
        .filter((nft) => Math.random() > 0.7) // Only some NFTs have lending offers
        .map((nft) => ({
          id: `listing-${nft.id}`,
          nftId: nft.id,
          nftName: nft.name,
          nftImage:
            nft.image || `https://via.placeholder.com/150?text=${nft.name}`,
          collection: collectionWithDescription.name,
          lender: nft.owner || 'unknown',
          loanDuration: Math.floor(Math.random() * 14) + 1, // 1-14 days
          interestRate: Math.random() * 20, // 0-20%
          collateralRequired:
            collectionWithDescription.floorPrice * (1 + Math.random() * 0.5), // 100-150% of floor price
          listedAt:
            Math.floor(Date.now() / 1000) - Math.floor(Math.random() * 604800), // Listed within the last week
          floorPrice: collectionWithDescription.floorPrice,
        }));

      setLendingOffers(offers);
    } catch (error) {
